...
8. **Hand-off:** สร้างไฟล์ `INDEX_<Module>.sql` (separate file — **ไม่รวมใน PACK_INSTALL อีกต่อไป**) — Header ระบุ install order: "Run AFTER PACK_INSTALL". ส่ง Index Strategy table ให้ orchestrator ฝังใน `DB_SPEC_<Module>.md` (ใต้ Data Dictionary ของแต่ละ Table — ดูกฎ "Per-Table Index Detail" ด้านล่าง):
   ...
   **🚨 No GO statements (MSSQL — บังคับ Rule 10):** ห้ามใช้ `GO` ใน INDEX file — `CREATE INDEX` ทุกตัวเป็น standalone statement ที่ใช้ `;` (semicolon) terminator ได้ ไม่ต้องการ batch separator
   ...
   -- 🚨 NO GO statements (MSSQL) — รองรับ sqlcmd / programmatic deploy
   -- ============================================================
   USE <db_name>;
   ...
   **Lint Check ก่อนส่ง (MSSQL only):** Grep INDEX file หา `^\s*GO\s*$` (standalone GO) — Pass = 0 occurrences. หากเจอ → ต้องลบทิ้งก่อน finalize
